...
class ImageProcessor():
    ...
    def process_frame_threaded(self,frame,threshold,return_processed=False):
        ...
        future_list = []
        for item in self.item_manager.enabled_items():
            future = self.executor.submit(self._process, item, scaled_canny, threshold)
    ...
    def _process(self,item,frame,threshold):
        template = item.template
        h,w,_ = item.shape
